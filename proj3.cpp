...
Node* Del(Node* root, int comp_num) {
    if (root == NULL) {
        printf("No such component - Comp # %d\n", comp_num);
        return root;
    }
    if (comp_num < root->comp_num) {
        root->left = Del(root->left, comp_num);
    }
    else if (comp_num > root->comp_num) {
        root->right = Del(root->right, comp_num);
    }
    else {
        if (root->left == NULL) {
        ...
        root->right = Del(root->right, temp->comp_num);
    }
    return root;
}
...
void print_inventory_helper(Node* root) {
    if (root == NULL) {
        return;
    }
    print_inventory_helper(root->left);

    printf("%-6d %-15s %-20d %-5d", root->comp_num, root->name, root->stock, root->reorder);
